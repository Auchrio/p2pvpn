...
1. [Network Configuration Files](#config-files)
2. [Peer Discovery (libp2p DHT)](#peer-discovery)
3. [NAT Traversal with AutoRelay/AutoNAT](#nat-traversal)
4. [Stream Establishment & VPN Protocol](#stream-protocol)
5. [IP Address Assignment](#ip-assignment)
6. [Configuration Propagation (GossipSub)](#config-propagation)
7. [TUN Interface Management](#tun-interface)
8. [Authentication & Signature Verification](#authentication)
9. [Whitelist Enforcement](#whitelist)
...
Network configuration files (`.conf`) store all parameters needed to start a daemon or join an existing network. They are generated by `network create` and can be loaded by `daemon start` and `network join` via the `--config` flag.

### File Format

The config file is a simple KEY=VALUE text format, readable and editable by hand:

```ini
# p2pvpn network configuration
# Generated by: p2pvpn network create

NETWORK_PUB=<hex-ed25519-public-key>
NETWORK_PRIV=<hex-ed25519-private-key>
...
**Key fields:**
- `NETWORK_PUB` (required): Network identity public key (shared with all peers)
- `NETWORK_PRIV` (optional): Network identity private key (authority only; kept secret)
- `CIDR`: Virtual IP block for address assignment
- `PREFERRED_IP`: Request a specific virtual IP (leave empty for auto-assign)
- `HOST_LOCKED`: Require signed config updates (true/false)
- `LISTEN_PORT`: libp2p listen port (0 = OS chooses random)
- `BOOTSTRAP_PEERS`: Comma-separated multiaddrs for initial peer contacts

### Loading and Merging

When a config file is loaded:
1. All fields from the file are parsed into a `NetConf` struct
2. If CLI flags are also provided (e.g., `--cidr`, `--port`), they **override** the config file values
3. The final `daemon.Config` is populated with merged values
...
# Override CIDR but keep other settings from config
sudo p2pvpn daemon start --config mynet.conf --cidr 10.50.0.0/24
```

### Implementation

Config files are handled by the `netconf` package (`utils/netconf/netconf.go`):

- `Load(path string) (*NetConf, error)` — Parses a config file
- `(nc *NetConf) Save(path string) error` — Writes a config file with mode 0600 (root-readable only)
- `(nc *NetConf) BootstrapPeerList() []string` — Splits comma-separated BOOTSTRAP_PEERS into a slice

---

## NAT Traversal with AutoRelay/AutoNAT

### The Problem: CGNAT and Double NAT

When two peers are behind different NATs (especially CGNAT — Carrier-Grade NAT on mobile hotspots), direct peer-to-peer connections fail because:

1. **NAT obscures addresses**: Each peer only knows its private IP (e.g., 192.168.x.x, 10.0.x.x), not the public IP of the hotspot/router
2. **CGNAT blocks hole-punching**: Carrier-grade NAT does not preserve source port, so coordinated hole-punching (which libp2p uses) often fails
3. **DHT advertisements are useless**: When peer B queries the DHT and finds peer A, the addresses listed are A's private IPs — B cannot route to them

### The Solution: AutoRelay + AutoNAT + Hole-Punching

p2pvpn enables a four-layer NAT traversal stack:

#### 1. NATPortMap

Attempts to open a hole in the NAT via **UPnP** (Universal Plug and Play) or **NAT-PMP** (NAT Port Mapping Protocol). If successful, the node becomes directly reachable without a relay.

```go
libp2p.NATPortMap()  // In p2p.New() options
```

**Pros:** Direct, lowest latency, lowest overhead
**Cons:** Many consumer routers don't support UPnP; almost no CGNAT supports it

#### 2. EnableRelay

Allows the node to use **libp2p relay nodes** as a circuit to relay traffic when direct connections fail.

```go
libp2p.EnableRelay()
```

A relay node is just a publicly reachable peer that agrees to forward traffic. When A wants to reach B through a relay R:
```
A → R → B (circuit connection)
```

**Pros:** Works even behind restrictive NAT/CGNAT
**Cons:** Higher latency, throughput limited by relay node

#### 3. EnableAutoRelayWithPeerSource

Makes the node **automatically find and reserve relay slots** when AutoNAT determines it is behind NAT. The `peerSource` closure feeds candidate relay nodes from the DHT routing table:

```go
peerSource := func(ctx context.Context, num int) <-chan peer.AddrInfo {
	// Yields peers from DHT routing table as potential relays
...
**How it works:**
1. When the node starts, AutoNAT probes its reachability (asks public nodes if they can connect back to you)
2. If not reachable, AutoRelay kicks in
3. AutoRelay calls `peerSource` to get candidates from the DHT routing table
4. Tries to connect to a candidate and register a relay slot
5. Advertises a relay address like `/ip4/relay-public-ip/tcp/4002/p2p-circuit/p2p/<your-peer-id>`

Other peers can now connect through this relay.

#### 4. EnableHolePunching

After two peers connect through a relay, the **hole-punching subsystem** attempts to upgrade the relay connection to a direct connection:
...
**How it works:**
1. Both peers agree on a "hole punch" protocol
2. They simultaneously open short-lived outbound connections to each other's registered addresses
3. If the NAT port mappings align, these connections "punch through" the NAT and connect directly
4. Future packets use the direct path, not the relay

#### 5. EnableNATService

Runs the AutoNAT **responder service** on this node. When other nodes ask "am I reachable?", this node probes them:

```go
libp2p.EnableNATService()
```

This is a good-citizen feature: it helps the network by letting peers determine their own reachability without running a dedicated AutoNAT service node.

### Code Changes

In `utils/p2p/p2p.go`, the `New()` function declares a `peerSource` closure that captures the DHT pointer, then adds all five options to the libp2p host:

```go
var kadDHT *dht.IpfsDHT
peerSource := func(ctx context.Context, num int) <-chan peer.AddrInfo {
	// Yields peers from DHT routing table for AutoRelay to try as relays
}

opts := []libp2pconfig.Option{
	libp2p.Identity(identityKey),
	libp2p.ListenAddrStrings(...),
	libp2p.NATPortMap(),              // Attempt UPnP/NAT-PMP
	libp2p.EnableRelay(),             // Allow relay circuits
	libp2p.EnableAutoRelayWithPeerSource(peerSource),  // Auto-find & reserve relays
	libp2p.EnableHolePunching(),      // Upgrade relays to direct
	libp2p.EnableNATService(),        // Help other peers with NAT detection
}
```

### Behavior Across Network Scenarios

| Scenario | Result |
|---|---|
| **Both on same WiFi** | mDNS finds them directly (no NAT traversal needed) |
| **Both on public IPs** | Direct connection via DHT discovery |
| **One behind UPnP router** | NATPortMap opens port; the other connects directly |
| **Both behind CGNAT (e.g., mobile)** | AutoRelay finds a relay; hole-punch upgrades if conditions allow |
| **One unreachable, one relayable** | Relay connection, hole-punch attempted |
